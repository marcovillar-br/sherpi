...
| EP15 — Robustez de ingestão (PDF-imagem) | Detectar conteúdo em imagem: páginas sem texto (`image_only_pages` → pula extração) e páginas **mistas** (`image_heavy_pages`: texto + imagem dominante → extrai e avisa). Sem laudo "íntegro" falso (Nível 1; OCR é EP13). Corpus: `scanned_acao_cobranca`, `scanned_parcial`, `scanned_mista`. | S | pós-9 | ✅ |
